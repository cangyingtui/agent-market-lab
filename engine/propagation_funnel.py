...
def _sentiment(decisions: list[dict[str, Any]]) -> dict[str, float]:
    ...
    for decision in decisions:
        ...
        bucket = "positive" if label == "buy" or score >= 0.68 else "negative" if label == "not_buy" or score < 0.45 else "neutral"
...
def build_propagation_funnel(
    ...
    sentiment = _sentiment(decisions)
    avg_intent = sum(
    ...
    purchase_to_recommend = clamp(
        safe_float(configured.get("purchase_to_recommend_rate"), 0.08 + sentiment["positive"] / 500) * fission_factor,
        0.01,
        0.90,
    )
    negative_attrition = clamp(safe_float(configured.get("negative_attrition_rate"), 0.04 + sentiment["negative"] / 500), 0.0, 0.50)
