...
def evidence_competitors(evidence: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    ...
    for item in evidence_items(evidence, *PRODUCT_EVIDENCE_KEYS):
        ...
        if key in seen:
            continue
        seen.add(key)
        # Compute fallback brand_score
        brand = raw.get("brand") or "未知品牌"
        raw_brand_score = raw.get("brand_score")
        if raw_brand_score is not None:
            brand_score = safe_float(raw_brand_score, -1) if isinstance(raw_brand_score, (int, float)) else -1
        else:
            brand_score = -1
        if brand_score < 0:
            # Fallback: use source_urls count as a weak brand signal
            source_urls = raw.get("source_urls") if isinstance(raw.get("source_urls"), list) else []
            src_count = len(source_urls)
            brand_score = clamp(58 + min(src_count, 3) * 3, 40, 80) if brand not in {"未知品牌"} else 50

        # Compute fallback sales_channel
        sales_channel = raw.get("sales_channel") or raw.get("channel") or ""
        if not sales_channel:
            raw_channels = raw.get("channels") if isinstance(raw.get("channels"), list) else []
            if raw_channels:
                sales_channel = ", ".join(str(c) for c in raw_channels)
            else:
                sales_channel = "线上渠道"  # sensible default
...
def collect_competitors(snapshot: dict[str, Any], evidence: dict[str, list[dict[str, Any]]], plan_type: str) -> list[dict[str, Any]]:
    ...
    if not rows:
        rows.append({
            "id": "generic", "name": "同类竞品", "brand": "泛竞品", "price": None,
            "specifications": {}, "score": 0.8,
            "sales_channel": "综合渠道", "brand_score": 50.0,
            "source": "", "source_urls": [], "enrichment_status": None,
            "needs_human_review": False, "channels": [],
        })
    return rows[:1] if plan_type == "basic" else rows
...
def competitor_analysis_rows(competitors: list[dict[str, Any]], market_share_full: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(competitors, 1):
        ...
        # Compute brand_score fallback (same logic as radar _brand_score)
        brand_score = safe_float(item.get("brand_score"), -1)
        if brand_score < 0:
            ...
            if not brand or brand in {"未知品牌", "自定义竞品"}:
                ...
                brand_score = clamp(58 + min(source_count, 3) * 3 - review_penalty, 40, 80)

        # Compute sales_channel fallback
        sales_channel = item.get("sales_channel") or ""
        if not sales_channel:
            channels = item.get("channels") if isinstance(item.get("channels"), list) else []
            if channels:
                sales_channel = ", ".join(str(c) for c in channels)
            else:
                source = str(item.get("source") or "")
                sales_channel = "线上渠道" if source else ""
...
def param_importance_rows(product: dict[str, Any], aggregation: dict[str, Any], plan_type: str) -> list[dict[str, Any]]:
    ...
    # Semantic driver-to-param keyword mapping (abstract drivers → concrete param traits)
    DRIVER_KEYWORDS: dict[str, list[str]] = {
        "价格": ["价格", "性价比", "成本", "花费", "预算", "price"],
        "功能": [],  # "关注功能" applies to all params equally
        "可靠": ["防水", "防摔", "续航", "电池", "耐用", "寿命", "质保", "保修", "防护"],
        "质量": ["防水", "防摔", "续航", "电池", "耐用", "寿命", "质保", "保修", "防护"],
        "品牌": ["品牌", "口碑", "信誉"],
        "创新": ["智能", "AI", "算法", "新技术", "创新"],
        "便携": ["重量", "尺寸", "便携", "折叠", "体积"],
    }
    ...
    result: list[dict[str, Any]] = []
    total_driver_bonus = 0.0
    for item in params:
        name = str(item["name"])
        name_lower = name.lower()
        value_str = str(item.get("value", "")).lower()
        combined_text = name_lower + " " + value_str

        # Step 1: Character-level Jaccard (for exact keyword matches in param name)
        best_jaccard_signal = 0.0
        for driver, count in driver_counts.items():
            ...
            best_jaccard_signal = max(best_jaccard_signal, overlap_score * frequency)

        # Step 2: Semantic keyword matching (abstract drivers → concrete params)
        best_keyword_signal = 0.0
        for driver, count in driver_counts.items():
            driver_lower = driver.lower()
            for semantic_key, keywords in DRIVER_KEYWORDS.items():
                if semantic_key not in driver_lower and semantic_key != "功能":
                    continue
                # "关注功能" matches all params at a moderate level
                if semantic_key == "功能":
                    best_keyword_signal = max(best_keyword_signal, 0.35 * (count / max_driver if max_driver else 0.0))
                    continue
                for kw in keywords:
                    if kw.lower() in combined_text:
                        frequency = count / max_driver if max_driver else 0.0
                        best_keyword_signal = max(best_keyword_signal, 0.6 * frequency)
                        break

        # Step 3: Weight-proportional fallback bonus when no match found
        weight = safe_float(item.get("weight"), 3.0)
        weight_ratio = weight / 5.0
        if best_jaccard_signal <= 0 and best_keyword_signal <= 0:
            # Distribute a small weight-proportional bonus so params don't all look identical
            weight_fallback = 0.12 * weight_ratio
            best_keyword_signal = weight_fallback

        best_signal = max(best_jaccard_signal, best_keyword_signal)
        driver_bonus = round(best_signal * 25, 1)  # continuous 0–25
        total_driver_bonus += driver_bonus
