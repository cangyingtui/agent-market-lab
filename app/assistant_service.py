...
def build_system_prompt() -> str:
    return (
        ...
        "最重要的原则：只回答用户实际问的问题，不要介绍用户没有问到的字段、页面或指标，不要输出固定套路或模板。"
        "用户问字段/参数怎么填，就解释这个字段；用户问业务建议（如定价、竞品、人群、卖点），就结合项目情况直接给建议；用户只是寒暄或问你是谁，就简短自我介绍。"
...
def call_llm(
    ...
    user_prompt = (
        ...
        f"【项目上下文】{compact(project_context, 1500)}\n\n"
        "请只回答「用户问题」本身，语气用“您/贵公司”，最多 400 字。\n"
        "用户问字段/参数/指标就直接解释；问业务建议（定价、竞品、人群、卖点等）就结合项目上下文给建议；寒暄或问你是谁就简短自我介绍。\n"
        "禁止输出“字段用途/填写要求/业务原因/下一步”这类固定模板，禁止介绍用户没问的字段或页面。"
    )
